...
function normalizeTextSpacing(str) {
  ...
  let normalized = str
    ...
    .map((line) => {
      let s = line;
      // Remove any whitespace (incl. NBSP) between a word and punctuation (prevents line-break before punctuation)
  ...
  // Global cleanup: catch any residual word+space+punct across paragraphs
...
// CRITICAL FIX: Pass Uint8Array directly to pdf-merger-js, NOT Buffer
// Issue #106: Buffer.from(arrayBuffer) loses data → blank pages
app.post('/api/compose', auth, async (req, res) => {
  try {
    ...
    // Save merged PDF using pdf-lib
    const pdfBytes = await mergedPdf.save();
    const totalPages = mergedPdf.getPageCount();
    console.log(`[Compose API] ✓✓✓ Merged PDF created successfully: ${pdfBytes.length} bytes, ${totalPages} total pages ✓✓✓`);
